...
\begin{definition}
	The matrix $\omega = A_{\alpha}dt^{\alpha} \in \Omega_X(U)\otimes \Ocal_X(U)^{\oplus n}$ is called a \emph{connection 1-form}.
\end{definition}
...
\begin{figure}[h]\label{F:parallel-transport}
	...
	\end{center}
	\caption{Movement in the base tells us how to move in the fibers by solving differential equations. The above figure shows paths in the space being transported to the vector bundle locally by solving differential equations.}
\end{figure}

We describe these equations.
If $s_* = f^js_j$ then to we just solve 
\begin{equation}\label{E:parallel-transport}
\nabla_{\alpha} f^j = 0, \quad j=1,\ldots, n
\end{equation}
...
The integrability issue is the is saying that a solution of $\nabla_{\alpha}Y=0$ may not also be a solution of $\nabla_{\beta}Y=0$.
The first eauation is an equation for $\partial_{\alpha} Y$ and the second equation is an equation for $\partial_{\beta}Y$. 
It is not guaranteed that $\partial_{\alpha} \partial_{\beta}Y = \partial_{\beta}\partial_{\alpha} Y$. 
When we can do this, the equation is integrable. 

As lifting directions is really about general derivations, we formulate this commutation problem more generally.
...
where the pairing $\langle -,-\rangle$ is the one between $\Omega_X^2$ and $T_X\wedge T_X$.
