...
We have
 $$ \lbrace x' =0 \rbrace \supset \lbrace x'' =0 \rbrace \supset \lbrace x''' =0 \rbrace \supset \cdots $$

One of the big consequences of the Basis Theorem is the Noetherianity of the Kolchin topology.
That is every nested sequence of Kolchin closed subsets 
 $$ \Khat^n \supset \Gamma_1 \supset \Gamma_2 \supset \cdots $$
must terminate. 

\begin{theorem}
The Ritt basis property implies any infinite ascending chain of radial differential ideals terminates.
\end{theorem}
\begin{proof}
...
The algebraic object associated to this space is the ring 
 $$ K\lbrace x_1,\ldots, x_n\rbrace/[f_1,\ldots,f_e],$$
and we try to read off properties of the space from 
...
In fact, although we know for every radical differential ideal $I$ there exists a finite set $u_1,u_2,\ldots,u_e$ such that the radical of the $\partial$-ideal generated by the $u_i$'s is $I$ (this is Ritt's Basis Theorem), we don't really know what these $u_i$'s are. 
In particular our algorithm only returns characteristic sequences $C$ which isn't a collection of $\partial$-radical generators but is sort of a discount groebner basis in that their division algorithm determines membership, 
The colon ideal of the characteristic set is the whole thing.

\begin{example}
	In the case of a prime ideal $P \subset K\lbrace x \rbrace$ by Lemma~\ref{L:structure-theorem} there exists some irreducible $g$ such that $P = J_g$ where $J_g = (\lbrace g \rbrace_{\partial}\colon S_g)$. 
\end{example}
...
\begin{theorem}[Pseudodivision Algorithm]
...
The algorithm above we call the pseudodivision algorithm and we use the notation $\red_g(f) = \widetilde{f}$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{$D$-schemes}\ref{S:d-schemes}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
