...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Representation and Character Varieties}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{remark}
	The point of using an algebraic stack here is not to be a jerk but to state that there is a serious mathematical issue that we need to contend with. 
	It's very complicated and two complicated theories for which there exist entire books written about them are devoted to dealing with this issue. 
	The first approach is Geometric Invariant Theory for which there is the famous book \cite{Mumford1994}. 
	The second would be the theory of Algebraic Stacks for which there is an entire Stacks Project. 
...
We have another example for which I won't give a proof. \taylor{add reference, maybe Loray}
\begin{example}
...
\subsection{$\partial$-Finite Generation}
We will set a $\partial$-ideal $I$ is \emph{$\partial$-finitely generated} if and only if there exist some $f_1,\ldots,f_n \in R$ such that $I=[f_1,\ldots,f_n]_{\partial}$.
This is a terrible notion in $\partial$-algebra. 

The next example is one of the product of two $\partial$-finitely generated ideals which is not finitely generated. 

\begin{example}
	We will see later that the ideal $\lbrace x y\rbrace_{\partial}$ is not $\partial$-finitely generated.
	We have $[x][y] = \langle x^{(i)}y^{(j)} \colon i,j \geq 0 \rangle = \lbrace xy \rbrace_{\partial} = [x]_{\partial} \cap [y]_{\partial}$.
\end{example}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Univariate Pseudodivision Algorithm}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We are going to give a preliminary definition. 
We are going to need to redo this computation later.
Let $R$ be a $\partial$-ring and let $f \in R\lbrace x \rbrace$. 
\begin{definition}
	\begin{enumerate}
		\item The \emph{leader} $\ell_f$ of $f$ is the highest derivative of $x$, $x^{(r)}$ appearing in $f$ (i.e. such that $\partial f/\partial x^{(r)} \neq 0$). 
		\item The \emph{separant} of $f$ is $S_f:=\partial F/\partial \ell_f$. 
		\item The \emph{initial} of $F$ is the coefficient $I_F$ of the leader, in other words
		 $$ f = a_0 + a_1 \ell_f + \cdots + a_d \ell_F^d, \quad I_f = a_d $$
		where if $\ell_f = x^{(r)}$ then $a_j \in R\lbrace x,x',\ldots,x^{(r-1)}]$. 
	\end{enumerate}
\end{definition}
...
\begin{remark}
	The separant is really going to be a source of a lot of our computational troubles in differential algebra that don't appear in algebra. 
\end{remark}

We now define a term order on $K\lbrace x \rbrace$ induces by the lexicographic ordering such that $x^{(r)} \prec x^{(r+1)}$ for all $r\geq 0$. 
...
The following examples shows us that there eare changes of 
\begin{example}
	Consider the chain of ideals in $K\lbrace x \rbrace$ gives a non-trivial infinite ascending chain 
...
The Ritt-Raudenbacher basis theorem implies Noetherianity.
\begin{theorem}
...
%%%%%%%%%%%%%%%%%%%%%%%
\section{Levi's Lemmas on $[y^n]$ and $[xy]$}
%%%%%%%%%%%%%%%%%%%%%%%
This section is about nilpotents which are largely ignored in modern differential algebra. 
This is sad. 
